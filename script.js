...
async function addExpense(){const year=Number(document.querySelector('#expenseYear').value),amount=Number(document.querySelector('#expenseAmount').value||0),description=document.querySelector('#expenseDesc').value.trim();const {error}=await sb.from('expenses').insert({year,expense_date:document.querySelector('#expenseDate').value||nowDate(),description,amount});if(error){msg(error.message);return;}msg('খরচ সংরক্ষণ হয়েছে।',true);document.querySelector('#expenseForm').reset();await loadAdmin();await loadPublic();}
